...
int gate_loader_init(const char *csv_path) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (parse_gate_line(line, &patch, &channel, &gate) == 0) {
            /* Validate and store */
            if (entries_loaded >= MAX_GATE_ENTRIES) {
                fprintf(stderr, "[gate_loader] Warning: Max entries reached (%d), ignoring rest\n",
                        MAX_GATE_ENTRIES);
                break;
            }
            
            g_gates[entries_loaded].patch = patch;
            g_gates[entries_loaded].channel = channel;
            g_gates[entries_loaded].gate_samples = gate;
            entries_loaded++;
        } else {
            fprintf(stderr, "[gate_loader] Warning: Failed to parse line %d: %s\n",
                    line_num, trimmed);
        }
    }
